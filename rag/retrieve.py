...
class AzureQdrantVectorStoreProvider(AbstractVectorStoreProvider):
    ...
    """
    def __init__(self, embedding_provider: Optional[AbstractEmbeddingProvider] = None):
        logger.info("Initializing Azure + Qdrant hybrid vector store")
        ...
        logger.info("Hybrid vector store initialized. Qdrant collection: %s", self.collection)
    ...
    def index_chunks(self, chunks: List[DocumentChunk]) -> int:
        ...
        logger.info("Indexing %d chunks into Qdrant + Azure AI Search", len(chunks))
